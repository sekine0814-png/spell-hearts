...
function renderDressupCurrent(hall){
  ...
  current.innerHTML=`<b>現在の装備</b><span>グー：${dressupSeriesName(equipped.rock)}</span><span>チョキ：${dressupSeriesName(equipped.scissors)}</span><span>パー：${dressupSeriesName(equipped.paper)}</span><span>アンプリファイア：${dressupSeriesName(equipped.amplify)}</span>`;
}
...
itemExchangeStyle.textContent+='.dressup-current{display:flex;flex-wrap:wrap;justify-content:center;gap:6px 12px;margin:0 auto 15px;padding:9px 12px;border:1px solid rgba(216,174,78,.48);border-radius:4px;background:rgba(5,6,11,.58);color:#d9ca9f;font:12px "Yu Gothic",sans-serif}.dressup-current b{width:100%;color:#ffe9a6;font:14px Georgia,"Yu Mincho",serif;letter-spacing:.1em}.dressup-current span{color:#e6d39b}.dressup-owned-items button.equipped{border-color:#ffe584;box-shadow:inset 0 0 16px rgba(255,225,116,.2),0 0 12px rgba(255,202,73,.3)}.dressup-owned-items button small{display:block;margin-top:4px;color:#fff0a5;font:12px "Yu Gothic",sans-serif;letter-spacing:.08em}@media(max-width:600px){.dressup-current{gap:4px 7px;margin-bottom:10px;padding:7px;font-size:10px}.dressup-current b{font-size:12px}.dressup-owned-items button small{font-size:9px}}';
