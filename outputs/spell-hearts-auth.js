...
import {
  ...
  linkWithCredential, EmailAuthProvider, signOut, sendPasswordResetEmail
...
function updateLoginButton(){
  ...
  button.textContent=(currentUser.email||'アカウント').split('@')[0];
...
function makeModal(){
  ...
  form.onsubmit=async event=>{
    ...
    const email=panel.querySelector('.auth-email').value.trim(), password=panel.querySelector('.auth-password').value;
