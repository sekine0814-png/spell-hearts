...
function equipCosmetic(category,series,item){
  ...
  const hall=document.querySelector('#dressupPanel');if(hall)renderDressupCurrent(hall);
  void saveCosmetics();
}
...
function dressupSeriesName(series){return series==='normal'?'ノーマル':battleSeriesNames[series]||series;}
function renderDressupCurrent(hall){
  const current=hall.querySelector('.dressup-current');if(!current)return;
  const equipped=cosmeticProfile.equipped.battle;
  const cardNames={rock:'グー',scissors:'チョキ',paper:'パー',amplify:'アンプリファイア'};
  current.innerHTML=`<b>現在の装備</b><div class="dressup-equipped-preview">${Object.keys(cardNames).map(card=>{const series=equipped[card]||'normal',image=battleArtFor(publicCosmetics(),card);return `<figure><img src="assets/${image}" alt="${cardNames[card]}：${dressupSeriesName(series)}"><figcaption>${cardNames[card]}<small>${dressupSeriesName(series)}</small></figcaption></figure>`;}).join('')}</div>`;
}
function showDressupOwnedItems(hall,category,series){
  ...
  content.innerHTML=`<h3>${seriesName}</h3><p>所持しているアイテム</p><div class="dressup-owned-items">${items.map(item=>{const equipped=category==='バトルカード'&&cosmeticProfile.equipped.battle[item.key]===series;return `<button class="${equipped?'equipped':''}" type="button" data-dressup-item="${item.name}"><img src="${item.image}" alt="${category} ${item.name}"><span>${item.name}</span>${equipped?'<small>装備中</small>':''}</button>`;}).join('')}</div>`;
...
function installOpeningSpellDeckGuide(){
  const original=window.render;
  if(typeof original!=='function'||original.openingSpellDeckGuideInstalled)return;
  const enhanced=function(...args){
    const result=original.apply(this,args);
    if(typeof g!=='undefined'&&g?.phase==='opening')document.querySelector('#cSpell')?.classList.remove('opening-spell-deck');
    return result;
  };
...
function startChapterOne(){
  ...
  const syncSceneBackdrop=()=>{
    if(!sceneBackdrop)return;
    const source=scene.classList.contains('night-village')?'assets/story-village-night.webp':scene.classList.contains('village-scene')?'assets/story-village.webp':'assets/story-training-ground.webp';
    if(!sceneBackdrop.src.endsWith(source))sceneBackdrop.src=source;
  };
  if(!scene.dataset.backdropSyncInstalled){new MutationObserver(syncSceneBackdrop).observe(scene,{attributes:true,attributeFilter:['class']});scene.dataset.backdropSyncInstalled='true';}
  syncSceneBackdrop();
...
itemExchangeStyle.textContent+='.dressup-current{margin:0 auto 15px;padding:9px 12px;border:1px solid rgba(216,174,78,.48);border-radius:4px;background:rgba(5,6,11,.58);color:#d9ca9f;text-align:center}.dressup-current>b{display:block;margin-bottom:7px;color:#ffe9a6;font:14px Georgia,"Yu Mincho",serif;letter-spacing:.1em}.dressup-equipped-preview{display:flex;justify-content:center;gap:10px}.dressup-equipped-preview figure{width:78px;margin:0}.dressup-equipped-preview img{display:block;width:78px;height:103px;object-fit:cover;border:1px solid rgba(255,225,132,.7);border-radius:3px;box-shadow:0 3px 9px #000}.dressup-equipped-preview figcaption{margin-top:3px;color:#fff0bd;font:12px Georgia,"Yu Mincho",serif;white-space:nowrap}.dressup-equipped-preview small{display:block;margin-top:1px;color:#d5bf84;font:9px "Yu Gothic",sans-serif;overflow:hidden;text-overflow:ellipsis}.dressup-owned-items button.equipped{border-color:#ffe584;box-shadow:inset 0 0 16px rgba(255,225,116,.2),0 0 12px rgba(255,202,73,.3)}.dressup-owned-items button small{display:block;margin-top:4px;color:#fff0a5;font:12px "Yu Gothic",sans-serif;letter-spacing:.08em}@media(max-width:600px){.dressup-current{margin-bottom:10px;padding:7px}.dressup-current>b{margin-bottom:5px;font-size:12px}.dressup-equipped-preview{gap:5px}.dressup-equipped-preview figure{width:56px}.dressup-equipped-preview img{width:56px;height:74px}.dressup-equipped-preview figcaption{font-size:10px}.dressup-equipped-preview small{font-size:8px}.dressup-owned-items button small{font-size:9px}}';
...
itemExchangeStyle.textContent+='#itemExchangePanel .dressup-series-list{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;width:min(100%,660px);margin:auto}#itemExchangePanel .dressup-series{width:auto;min-width:0;margin:0;padding:7px 4px 8px;gap:4px}#itemExchangePanel .dressup-series img{width:76px;height:96px}#itemExchangePanel .dressup-series span{font-size:13px;letter-spacing:.02em;white-space:nowrap}#itemExchangePanel .dressup-series small{font-size:10px;white-space:nowrap}@media(max-width:600px){#itemExchangePanel .dressup-series-list{grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;width:min(100%,360px)}#itemExchangePanel .dressup-series{padding:6px}#itemExchangePanel .dressup-series img{width:70px;height:88px}#itemExchangePanel .dressup-series span{font-size:12px}#itemExchangePanel .dressup-series small{font-size:9px}}';
itemExchangeStyle.textContent+='@media(min-width:601px){#itemExchangePanel .item-exchange-book{width:min(96vw,1180px);max-height:calc(100vh - 28px);min-height:0;padding:32px 54px 24px;overflow:auto}#itemExchangePanel .item-exchange-detail{min-height:0;padding:25px 18px 12px}#itemExchangePanel .item-exchange-detail-content{min-height:390px;padding:20px}#itemExchangePanel .astrologian-items{grid-template-columns:repeat(4,minmax(0,1fr));gap:18px;width:min(100%,980px);margin:12px auto 0}#itemExchangePanel .astrologian-items button{padding:10px 8px 12px;font-size:17px}#itemExchangePanel .astrologian-items img{height:260px;margin-bottom:10px}#itemExchangePanel .astrologian-items small{display:block;margin-top:5px;font-size:12px;color:#cbb987}}';
...
chapterOneStyle.textContent+='.chapter-scene-backdrop{position:absolute;z-index:0;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;pointer-events:none}.chapter-one-scene:has(.chapter-scene-backdrop){background:#020509!important}.chapter-one-scene:has(.chapter-scene-backdrop):before{display:none}';
...
chapterOneStyle.textContent+='body.story-active:not(.story-cinematic) .top .btn{position:relative;z-index:200!important;pointer-events:auto!important}';
