...
function openAstrologianConfirm(item){
  ...
  dialog.querySelector('.exchange-confirm-yes').onclick=()=>{
    ...
    dialog.querySelector('.item-exchange-confirm-box').innerHTML=`<img src="${item.image}" alt="astrologian ${item.name}"><p>astrologian ${item.name}を交換しました！</p><button type="button" class="exchange-confirm-no">閉じる</button>`;
    dialog.querySelector('.exchange-confirm-no').onclick=close;
  };
}
