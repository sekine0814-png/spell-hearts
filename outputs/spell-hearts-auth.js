...
function openItemExchange(){
  ...
  if(!hall){
    ...
    hall.innerHTML='<div class="item-exchange-book" role="dialog" aria-modal="true" aria-labelledby="itemExchangeTitle"><button class="item-exchange-close" type="button" aria-label="閉じる">×</button><p class="item-exchange-kicker">ARCANE EXCHANGE</p><h2 id="itemExchangeTitle">アイテム交換所</h2><p class="item-exchange-copy">交換したいカードの種類を選んでください</p><div class="item-exchange-categories"><button type="button" data-exchange-category="グー"><img src="assets/rock.jpg" alt="グー"><span>グー</span></button><button type="button" data-exchange-category="赤チームのバトルカード"><img src="assets/red-battle-back.png" alt="赤チームのバトルカード裏面"><span>赤チーム<br>バトルカード</span></button><button type="button" data-exchange-category="青チームのスペルカード"><img src="assets/blue-spell-back.jpg" alt="青チームのスペルカード裏面"><span>青チーム<br>スペルカード</span></button></div><div class="item-exchange-detail" hidden><button class="item-exchange-detail-back" type="button">← カードの種類を選ぶ</button><div class="item-exchange-detail-content"></div></div><button class="item-exchange-return" type="button">召喚の門へ戻る</button></div>';
    ...
    hall.querySelectorAll('[data-exchange-category]').forEach(button=>button.onclick=()=>{
      ...
      hall.querySelector('.item-exchange-detail-content').innerHTML=`<h3>${name}</h3><p>このカードの着せ替えアイテムを表示します。</p><div class="item-exchange-empty">交換できるアイテムを準備中です</div>`;
